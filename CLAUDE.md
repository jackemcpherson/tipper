...
AFL match prediction CLI (`tipper`) combining MOV-Elo ratings with player-level PAV (Player Approximate Value) ratings. The CLI calls the Cloudflare D1 REST API directly and runs the engine locally. A companion Cloudflare Worker (`src/worker/`, revived in tipper#30 after the v3.2 retirement) publishes predictions on a cron with a native D1 binding.
...
**CLI (`src/cli/`)** — Commander-based. Reads configs from disk, calls orchestration functions directly via the D1 REST shim, formats output. Commands: `config {list,show,current,promote,diff,create}`, `backtest`, `predict`, `compare`, `publish` (manual/break-glass write of a round's predictions to the `match_predictions` D1 table — scheduled publishing is the Worker's job).

**Worker (`src/worker/`)** — Cloudflare Worker cron publisher (tipper#30). `plan.ts` is the pure decision core (`publishPlan`/`healthStatus`: 7-day fixture window, daily/hourly/15-min Thursday-announcement cadence, freeze at first kickoff, Melbourne clock via `Intl`); `tick.ts` is the integration seam (`runPublishTick`: state query → plan → per-round fail-soft `runPrediction` + `upsertPredictions`); `index.ts` is the logic-free shell (scheduled handler + `GET /health`). `baked-config.ts` is GENERATED by `bun run bake-config` from `configs/_current.json` and committed — regenerate it after every config promotion. Worker code must use only Web Standard APIs (no `node:` imports). Prod deploys are GitOps via cloudflare-infra; `wrangler.jsonc` is local-dev only.
