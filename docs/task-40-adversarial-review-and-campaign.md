...
| A3 | `t40-ha-neutral`, `t40-ha-bucket`; flat `t40-ha-070/090/100` | Neutral uses Task 33's declared-home-ground exception and zero HA. Bucket uses neutral 0, same-state derby 20, home-local/away-interstate 110, otherwise 80 rating points. Flat doses change only prediction HA. I and C. |
| A4 | `t40-offset-v4`; `t40-offset-k08/k16`; `t40-offset-c0/c1`; `t40-offset-tail` | Clone v4. Smaller k is 8/16 versus 32; carries are 0/1 versus 0.5. Tail learns the same residual model but applies its offset only when pre-offset absolute margin exceeds 24, with a sign-preserving cap. I and C. |
