...
**Date:** 2026-04-26 **Starting baseline:** `elo-only-tuned-v1b` (K=25, HA=160,
RTM=0.10, weight_elo=1.0, sigma=36) **Final model:** `pavfix-blend-w06`
(weight_elo=0.6, pav_calibration_slope=6.986)

---

## Part A: Recalibrated Slope

Calibration regression on 2021-2024 (846 matches), using corrected PAV formula
(Task 13 defence fix applied).
...
The 28x difference reflects the removal of the 100x defence inflation. With the
bug, defensive PAV dominated the total signal, and the slope had to be very
small to compensate. The corrected slope is in the expected range for converting
PAV differentials (~40-point range) to Elo-equivalent units.

PAV-Elo diff correlation: **0.808**: moderate complementarity. PAV and Elo
correlate because both measure team quality, but PAV captures player-level
...
weight_elo=0.6, and every blend config beats Elo-only. This result is a complete
reversal from v1 Task 1, where more Elo weight was always better: because v1 was
blending against broken PAV.

The improvement at the optimum (-0.0120 LogLoss) is larger than the entire v1
tuning process improvement (-0.015 from blended-v1 to elo-tuned-v1).
...
**Both LogLoss and Brier CIs exclude zero.** This result is the first parameter
change in the project to achieve statistical significance at 95% confidence.
Task 11 showed that none of the v1/v1.5 decisions could achieve this: the effect
sizes were too small relative to sampling noise. The corrected PAV signal is
large enough to clear the noise floor.
...
PAV helps in four of five years. The 2021 underperformance (+0.0055) matches
expectations because it is the first test year. The Bayesian prior uses limited
cold-start PAV data. The improvement strengthens over time as PAV state
accumulates (2024: -0.0315 is the largest single-year delta).

## Decision: Outcome A: PAV Genuinely Contributes

The v1.5 decision to bench PAV was wrong. The 100x defence bug was masking real
...
### Updated V1.5 Parameters to Task 15 Parameters
