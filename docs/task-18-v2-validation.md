...
**Date:** 2026-04-30 **Model under test:** `pavfix-blend-w06` (v2)
**Parameters:** K=25, HA=160, RTM=0.10, weight_elo=0.6,
...
v2 is the Task 15 PAV blend. Tasks 16 (contextual K) and 17 (venue HA) did not
clear their gating criteria and are not included.

The single v1.5 change restores corrected PAV at a 40% blend weight. It produces
a statistically significant improvement with a LogLoss CI excluding zero. It
places tipper ahead of every Squiggle source on 2026 probabilistic accuracy.
...
### Cross-Era Generalisation

The LogLoss gap between backwards (0.8845) and in-sample (0.8607) is **0.024**.
This gap slightly exceeds the 0.02 threshold used in v1.5 validation. The v1.5
Elo-only model had 0.8841 on the same window: essentially identical. The gap is
attributable to fitting the calibration slope to 2021-2024 data, producing mild
miscalibration on the earlier era. Acceptable.

### Forward Generalisation

77.8% tips and 0.8029 LogLoss on 2026 is excellent. The v1.5 model had 65.0%
tips and 0.887 LogLoss on 2026 (60 matches). V2 substantially outperforms v1.5
on forward data. PAV's ability to identify player-quality mismatches that Elo
alone misses drives the improvement. Player-quality mismatches that Elo alone
misses.

---

## Table 2: V1.5 Versus V2 Comparison (In-Sample)
...
| MAE     | 27.21           | **26.75**      | -0.46   | Not available      | :       |
| RMSE    | 34.77           | **34.22**      | -0.55   | Not available      | :       |

LogLoss and Brier CIs both exclude zero. This result is the first statistically
...
| Not available | **Median**          | **74.6%** | **0.9603** |
| ...           |                     |           |            |
| 30            | Elo Predicts!       | 61.9%     | 1.0123     |

Tipper v2 leads the field on LogLoss by 0.086 over the next-best source
(Winnable). On tip%, Wheelo Ratings leads at 82.5% but with much worse LogLoss
(0.938), indicating overconfident correct tips.

**Caveat:** This result covers 63 early-season matches. PAV's player-quality
signal may have unusual value before 2026 form changes emerge. Established 2025
player hierarchies still carry forward. The lead may narrow as the season
progresses and Elo catches up.

---

## Table 4: per-Team Audit (V1.5 Versus V2)
...
Essendon). West Coast and North Melbourne remain the worst-rated teams: these
are era-effect problems that PAV alone cannot fix.
...
| Forward generalisation holds                        | **PASS**: 0.803 vs v1.5's 0.887 on 2026               |
...
forward-validated, and produces the best probabilistic accuracy in the Squiggle
field. The per-team criterion fails because PAV does not address era-effect
misrating. Contextual K targeted that problem but could not clear the noise
floor.

The marginal 0.024 cross-era result does not block a 0.02 threshold. The fitted
2021-2024 calibration slope explains the gap. The backwards window (2018-2019)
has different PAV characteristics. The v1.5 Elo-only model showed 0.011 on the
same test: the increase reflects PAV's era sensitivity, not Elo degradation.
...
1. Extend 2026 monitoring: the early-season lead is large but may narrow.
   Re-evaluate at mid-season (Round 12+) with 100+ matches.

2. Contextual K-factor: Task 16 showed a directionally correct but sub-threshold
   effect. With more data (2026 completing, potentially 1200+ matches), the same
   0.0005 delta might clear the noise floor.

3. Venue HA with longer derivation: Task 17 failed because the derivation window
