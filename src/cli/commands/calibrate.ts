...
import { compOption, configOption, jsonOption, noCacheOption } from "../flags.js";
...
export const calibrateCommand = new Command("calibrate")
  ...
  .action(
    async (opts: { comp: CompetitionCode; config?: string; json: boolean; cache: boolean }) => {
      ...
      const result = await runCalibration(db, config, opts.comp, cache);
