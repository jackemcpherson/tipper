...
export const monitorCommand = new Command("monitor")
  ...
  .action(
    async (opts: { season?: number[]; comp: "AFLM" | "AFLW"; cache: boolean; log: boolean }) => {
      ...
      if (!season) {
        console.error("Error: monitor requires --season <year>, e.g. --season 2026");
        process.exit(1);
      }
      ...
      async function backtestSeason(configId: string) {
        ...
        const seasonConfig = {
          ...config,
          backtest: { ...config.backtest, test_seasons: [season as number] },
        };
      ...
      if (closeN === 0) {
        ...
        console.log(`  v3       ${closeV3}/${closeN} (${((closeV3 / closeN) * 100).toFixed(1)}%)`);
        console.log(`  v4       ${closeV4}/${closeN} (${((closeV4 / closeN) * 100).toFixed(1)}%)`);
        console.log(`  OD       ${closeOD}/${closeN} (${((closeOD / closeN) * 100).toFixed(1)}%)`);
        if (closePunN > 0) {
          console.log(
